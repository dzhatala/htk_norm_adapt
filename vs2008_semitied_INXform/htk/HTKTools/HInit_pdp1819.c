      ...
      if ( ncas > 0) {
         for (i=1,nObs=0; i<=ncas; i++) {
            ...
            if (segEnIdx - segStIdx + 1 >= nStates-2) {
			   ...
			   /*PrintObservation(0,&(ss->o),3); /**zoel ***/
...
LogFloat ViterbiAlign(int segNum,int segLen, IntVec states, IntVec *mixes)
{
   ...
   MakeTraceBack(segLen);
	...
	ShowMatrix("   zoel tranp ->",hmmLink->transP,4,4);
   ...
   if (trace & T_VIT|1) ShowP(1,lastP);  
   ...
   for (segIdx=2; segIdx<=segLen; segIdx++) {
      ...
      if (trace & T_VIT|1) ShowP(segIdx,lastP);   
   }
            ...
            if (hss.isCont && (uFlags&(UPMEANS|UPVARS)))/*PLAINHS or SHAREDHS*/
               while (GoNextMix(&hss,TRUE)) {
                  if (!IsSeenV(hss.mp->mean)) {
                     ...
                     TouchV(hss.mp->mean);
					 printf("zoel: State %d\n",istate);
					 ShowVector("old mean ma->mu:",ma->mu,3);
					 ShowVector("new mean hss.mp->mean:",hss.mp->mean,3);
                  ...
                  if (!IsSeenV(hss.mp->cov.var)) {
					 ...
					 ShowVector("new var hss.mp->cov.var:",hss.mp->cov.var,3);
