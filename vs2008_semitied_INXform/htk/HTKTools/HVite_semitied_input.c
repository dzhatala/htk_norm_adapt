...
char *hvite_version = "!HVER!HVite:   3.4.1 ZH[CUED 12/03/09]";
...
static int REPLAY_BUFF_LONG=3;
static FILE *n_stdout_file=NULL ; /**zoel: text file for writing 
						  recognition results, words and probs score**/
...
void ReportUsage(void)
{
   ...
   printf(" -Z s    replay buffer s seconds [3 default]   \n"); /**zoel */
   printf("\n\n");
}
...
{
	/**FILE *fid=fopen ("tmp\\testraw.txt","wb");*/
	FILE *fid=fopen ("tmp\\testraw.txt","wb");
	if (fid==NULL) {
		perror("??: ");
	}else {
		printf("Success 001\n");
		fprintf(fid,"%s ","test fprintf\n using c\n");
		fflush(fid);
		fclose(fid);
	}
}
int main(int argc, char *argv[])
{
   ...
   while (NextArg() == SWITCHARG) {
	...
	break;
	 ...
	 if (NextArg() != SWITCHARG)
	  ...
	  case 'Z' : /*zoel */
		 if (NextArg()!=INTARG)
            HError(3219,"HVite: -Z must be followed seconds in INT ");
         	  REPLAY_BUFF_LONG = GetChkedInt(1,256,s);
         break;
   ...
   if ( (n_stdout_file = fopen("tmp\\recog_results.txt","wb")) == NULL){
      perror("can't open rec results file ");
	  HError (10003,"zoel: can't open rec results for writing \n");
   }else {
	  printf("recog. results file tmp\\recog_results.txt successfuly opened\n");
	  print_time=time(NULL);
	  fprintf(n_stdout_file,"%i\nprinting ready\n",print_time);
	  fflush(n_stdout_file);
   }
   ...
   if (trace & T_MEM) {
	...
	if(n_stdout_file!=NULL){ /**zoel */
		fflush(n_stdout_file);
		fclose(n_stdout_file);
	 }
...
Boolean ProcessFileST(char *fn, Network *net, int utterNum, LogDouble currGenBeam, Boolean restartable)
{
   FILE *file;
   ParmBuf pbuf;
   BufferInfo pbinfo;
   ...
   /*zoel : passing internal volume scaler*/
   ...
   /*if (pbinfo.a != NULL && replay)  AttachReplayBuf(pbinfo.a, (int) (3*(1.0E+07/pbinfo.srcSampRate)));  zoel */
   if (pbinfo.a != NULL && replay)  AttachReplayBuf(pbinfo.a, (int) (REPLAY_BUFF_LONG*(1.0E+07/pbinfo.srcSampRate)));
   ...
   if (trace & T_TOP || fn==NULL) {
	...
	while(node!=NULL) {
		 ...
		 if (arc->end->word!=NULL){
			...
			if (n_stdout_file!=NULL){ /**zoel */
				fprintf(n_stdout_file, "%s ",arc->end->word->wordName->name);
				fflush(n_stdout_file);			
			}
		 }
	  ...
	  if (n_stdout_file!=NULL){ /**zoel */
		fprintf(n_stdout_file," ==  [%d frames] %.4f [Ac=%.1f LM=%.1f] (Act=%.1f)\n",nFrames,
             (aclk+lmlk)/nFrames, aclk,lmlk,(float)tact/nFrames);
		fflush(n_stdout_file);
	  }
      fflush(stdout);
