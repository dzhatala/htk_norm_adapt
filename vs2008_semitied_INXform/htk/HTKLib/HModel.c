...
/**PCA STUFF **/
static int is_PCA_filtered=0 ;/**zoel getconfbool PCA_FILTERED**/
static Vector pca_filtered_coefs=NULL ;/**zoel getconfstring**/
...
void InitModel(void)
{
   ...
   if (nParm>0){
	  ...
	  if (GetConfBool(cParm,nParm,"PCAFILTERED",&b)) { /**** zoel ***/
		  is_PCA_filtered = b; /** zoel **/
		  HError(-9001,"MFCC IS FILTERED USING PCA ONLY = %s ",b?"True":"False");
	  }
   }
}
...
/* @todo, fix this zoel modified form original version */
static Boolean in_filtered_pca_coeffs(int i){
	if(i==1) return FALSE ; /*first coeffs energy */
	if (i<25) return TRUE;
	return FALSE;
}
static LogFloat DOutP(Vector x, int vecSize, MixPDF *mp)
{
   ...
   sum = mp->gConst;
   for (i=1;i<=vecSize;i++) {
	   /** matlab can generate pca training files
	   but when online recognition ???? how to create pca files without matlab
	   ? so this two lines is 
	   **/
	   if(is_PCA_filtered){
		   if (in_filtered_pca_coeffs(i)) continue;
	   }
