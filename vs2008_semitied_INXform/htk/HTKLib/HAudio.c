...
static size_t to_narrow(const wchar_t * src, char * dest, size_t dest_len){
  size_t i;
  wchar_t code;
...
static void zmmapi_info1 (void){
#ifdef MMAPI_AUDIO
 
	int deviceCount = waveInGetNumDevs();
	HError(-1,"zmmapi_info1() get %d Input Audios",deviceCount);

    if ( deviceCount > 0 )
    {
		int idv;
        for (  idv = 0; idv < deviceCount; idv++ )
        {
			char name[1024];
			/*char destination[1024];*/
              int intDestLengthBytes=20;
			WAVEINCAPSW WaveInCaps;     
            MMRESULT result=waveInGetDevCaps( idv, &WaveInCaps, sizeof( WAVEINCAPS ) );
			intDestLengthBytes=sizeof(WaveInCaps.szPname);
			WideCharToMultiByte(CP_UTF8, 0, WaveInCaps.szPname, -1, (LPSTR)name, 
					intDestLengthBytes, NULL, NULL);
					printf("Input %i :  %s == <\n",idv, name);
        }
    }
...
static void InitAudi(AudioIn a, HTime *sampPeriod)
{
...
#ifdef MMAPI_AUDIO 
   {
      int i=0;
         ...
         HError(-1,"zoel in MMAPI_AUDIO InitAudi() "); 	  /**zoel**/
         /*trace=trace|T_AUD; */	  /**zoel**/
...
static void StartAudi(AudioIn a)
{
...
#ifdef MMAPI_AUDIO
   {
	  ...
	  zmmapi_info1();
	  printf("Starting Audio Input from active state = %d  WAVE_MAPPER %d \n",a->isActive,WAVE_MAPPER);
...
static MemHeap wavStack;     /* storage for waveforms --> backupreplaybuffer zoel */

#include <stdio.h>
#include <stdlib.h>

/* http://www.java2s.com/Code/C/File/Copyafile.htm */
static int fcp(const char *nf,const char *nto )
{
  FILE *from, *to;
  char ch;
  ...
  /* open source file */
  if((from = fopen(nf, "rb"))==NULL) {
    printf("Cannot open source file.\n");
    exit(1);
  }

  /* open destination file */
  if((to = fopen(nto, "wb"))==NULL) {
...
static BackupReplayBuffer(AudioOut a, short *audiobuf, int nSamples){
	time_t rawtime;
	char buffer [255];
	char *p = buffer;
	int count,length;
	...
	time (&rawtime);
#ifdef WIN32
	sprintf(buffer,"tmp\\rep_buf_%s",ctime(&rawtime) );
#else
	sprintf(buffer,"tmp/rep_buf_%s",ctime(&rawtime) );
#endif
	...
	if(fid==NULL ){
		perror("Failed");
		HError(10000,"Error creating file for replay backup");
	}
	HError(-1,"nsammples not sync. with PCM bit size\n");
	*/
	
	/*@todo error, amplitude over gain
	WriteHTKHeader (fid, nSamples, 625, 2, 0,NULL); /**sampP 1/16000 * 1e9 = 625 **/
	/*fwrite (audiobuf,sizeof(short),nSamples,fid);*/
	CreateHeap(&wavStack, "wavStack", MSTAK, 1, 1.2, 512, 4096); /* zoel */
	sampPeriod=625;
   w = OpenWaveOutput(&wavStack, &sampPeriod, nSamples);
   PutWaveSample(w,nSamples,audiobuf);
   CloseWaveOutput(w,HTK,buffer);
   fcp(buffer, "tmp\\test.htk");
   /*CloseWaveOutput(w,HTK,"tmp\\test.htk");*/\
...
void PlayReplayBuffer(AudioOut ao, AudioIn ai)
{
   ...
   if (ai->rbuf.inx <= ai->rbuf.outx){
	  BackupReplayBuffer(ao,ai->rbuf.data+ai->rbuf.outx,ai->rbuf.size - ai->rbuf.outx);
      /*PlayAudio(ao,ai->rbuf.data+ai->rbuf.outx,ai->rbuf.size - ai->rbuf.outx); zoel */
	  if (ai->rbuf.inx>0) {
		 BackupReplayBuffer(ao,ai->rbuf.data,ai->rbuf.inx);
         /*PlayAudio(ao,ai->rbuf.data,ai->rbuf.inx); zoel */
	  }
   } else{
	  BackupReplayBuffer(ao,ai->rbuf.data+ai->rbuf.outx,ai->rbuf.inx - ai->rbuf.outx);
      /*PlayAudio(ao,ai->rbuf.data+ai->rbuf.outx,ai->rbuf.inx - ai->rbuf.outx); zoel */
   }
}
