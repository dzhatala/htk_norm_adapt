sync
source env_aspire.sh				#use when aspire one linux shell is used
...
}' $labdir/*.lab | sort -r | uniq > phonems.lst

echo "ENTER " ; read
cp /dev/null hmm0/hmmdefs


for x1 in `cat phonems.lst` 
do 
	if [ "$x1" == " " ] 
	then
...
do
	...
	echo "creating hmm $x1 from hmm0/initial"
	cat hmm0/initial | sed s/initial/$x1/ >> hmm0/hmmdefs
done
