...
sync
#echo using datadir=$datadir ; echo ENTER ; read
...
cp full_herest_out/03/hmmdefs hmm7/
...
echo "hmm10..."
mkdir -p hmm10
cmd="$HTKTOOLS_DIR/HHEd  -T 1 -H hmm9/hmmdefs -M hmm10 mktri.hed monophones1_full.txt"
#echo $cmd ; eval $cmd
echo ENTER ; read
...
cmd="cp x240_train.lst mfc_triph.scp"
#cmd="cp now.lst mfc_triph.scp"

echo $cmd ; eval $cmd

prune_param="50.0 150.0 1000.0"

cmd="$HTKTOOLS_DIR/HERest  -T 1 -C configtrain.txt $trilab $prune  \
