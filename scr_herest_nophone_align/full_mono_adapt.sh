...
cmd="HHEd -B -H full_herest_out/04/hmmdefs  -M classes regtree.hed monophones1_full.txt"
#echo $cmd ; eval $cmd
#echo ENTER ; read
...
cmd="HERest -T 7 -C configtrain.txt -C config.semi -S adapt.scp $use_mlf \
 -u stw  -H full_herest_out/04/hmmdefs  -K xforms mllr1 -J classes  -z x.tmf  -h '*/m1*.mfc' -M tmp monophones1_full.txt"
#echo $cmd ; eval $cmd
#echo ENTER ; read


cmd="HERest -T 7 -C configtrain.txt -C config.global -S adapt.scp $use_mlf \
...
cmd="HERest -T 1 -C configtrain.txt -C config.semi -S adapt.scp -L gen_lab -u stw \
...
mkdir -p hmm17
cmd="HERest -T 1 -C configtrain.txt  -S adapt.scp -L gen_lab -u tmvw \
